...
func configure(instance: DigimonInstance, species: Dictionary, progression: DigimonProgressionService, show_vitals: bool = true) -> DigiCompactProfilePanel:
	...
	portrait_frame.custom_minimum_size = Vector2(136, 136)
	...
	portrait.custom_minimum_size = Vector2(122, 122)
	...
	var name := _label(display_name, 26, V2.WHITE, true)
	...
	var rank_level := _label("%s  ·  LV. %d" % [rank.to_upper(), instance.level], 14, accent, true)
	...
	chips.add_child(_pill("POT %d" % instance.potential, V2.PURPLE))
	chips.add_child(_pill("LINK %d" % instance.link, V2.CYAN))
	...
	if show_vitals:
		...
		var hp := _label("HP %d / %d" % [clampi(instance.current_hp, 0, max_hp), max_hp], 11, V2.GREEN, true)
		hp.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		vitals.add_child(hp)
		vitals.add_child(_label("SP %d / %d" % [clampi(instance.current_mp, 0, max_sp), max_sp], 11, V2.BLUE, true))
	return self
