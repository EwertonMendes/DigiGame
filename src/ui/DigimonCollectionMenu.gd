extends Control
class_name DigimonCollectionMenu
...
func _ready() -> void:
	...
	OverworldState.collection_changed.connect(_refresh_collection)
...
func _build() -> void:
	...
	_collection_panel.name = "CollectionPanel"
	_collection_panel.add_theme_stylebox_override("panel", UI.glass_panel(UI.CYAN, 0.80, 10))
	add_child(_collection_panel)
	var collection_margin := _margin(12, 12, 12, 12)
	...
	collection_margin.add_child(_collection_scroll)
...
func _refresh_collection() -> void:
	...
	_selected_index = clampi(_selected_index, 0, maxi(0, collection.size() - 1))
	for index in range(collection.size()):
		var instance: DigimonInstance = collection[index]
...
func _select_index(index: int) -> void:
	if index < 0 or index >= OverworldState.get_collection_instances().size():
		return
...
func _style_collection_selection() -> void:
	...
	for index in range(_buttons.size()):
		...
		if index < collection.size():
			var species := _database.get_by_seed(collection[index].species_seed)
...
func _refresh_details() -> void:
	...
	if collection.is_empty():
		var empty := _label("No Digimon in your collection.", 16, UI.MUTED)
...
func _layout() -> void:
	...
	_collection_panel.scale = Vector2.ONE * scale_factor
	_detail_panel.scale = Vector2.ONE * scale_factor
	if compact:
		var collection_h := minf(198.0, height * 0.30)
		_collection_panel.position = origin + Vector2(16.0, header_h) * scale_factor
		_collection_panel.size = Vector2(width - 32.0, collection_h)
		_detail_panel.position = origin + Vector2(16.0, header_h + collection_h + 10.0) * scale_factor
		_detail_panel.size = Vector2(width - 32.0, height - header_h - collection_h - 26.0)
		...
		var collection_w := clampf(width * 0.255, 270.0, 320.0)
		_collection_panel.position = origin + Vector2(16.0, header_h) * scale_factor
		_collection_panel.size = Vector2(collection_w, height - header_h - 16.0)
		_detail_panel.position = origin + Vector2(28.0 + collection_w, header_h) * scale_factor
		_detail_panel.size = Vector2(width - collection_w - 44.0, height - header_h - 16.0)
		_collection_scroll.horizontal_scroll_mode = ScrollContainer.SCROLL_MODE_DISABLED
		_collection_scroll.vertical_scroll_mode = ScrollContainer.SCROLL_MODE_AUTO
		_collection_grid.columns = 1
