...
func _rebuild_skill_list() -> void:
	...
	for action: Dictionary in actions:
		...
		button.add_theme_color_override("font_disabled_color", Color(UI.MUTED.r, UI.MUTED.g, UI.MUTED.b, 0.46))
...
func _layout() -> void:
	...
	var skill_width := 360.0
	if portrait_mobile:
		skill_width = minf(physical.x - 20.0, 420.0)
	...
	if portrait_mobile:
		...
		_skill_panel.position = Vector2(262.0 * ui_scale, 62.0 * ui_scale)
	else:
		_skill_panel.position = Vector2(276.0 * ui_scale, 190.0 * ui_scale)
