...
func _play_sequence(sequence_id: int) -> void:
	...
	# Each Digimon owns an independent progression timeline. The result shell and
	# its V2 layout remain static while XP, level-ups and rewards animate in
	# parallel, so a multi-level card never blocks the rest of the squad report.
...
func _cancel_parallel_card_animations(cancelled_sequence: int) -> void:
	...
	_pending_parallel_cards = 0
	if had_pending:
		# Wake the orchestration coroutine so it observes the new sequence id and
		# exits instead of remaining suspended on a cancelled result animation.
		parallel_card_batch_finished.emit(cancelled_sequence)


func _on_continue_pressed() -> void:
	if _animating:
		_cancel_parallel_card_animations(_sequence_id)
	super._on_continue_pressed()
