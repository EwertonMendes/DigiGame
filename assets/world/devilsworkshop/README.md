...
MCBlocks has been retired from DigiGame. DigiLab is the first Central City service to receive a dedicated exterior (`assets/world/tblack/digilab/digilab.png`); its authored door is aligned to a 0054 forecourt and drives the seamless interior threshold. The dedicated `digilab/` folder also carries `digilab-door-semi-open.png` and `digilab-door-open.png`, which are played in sequence before entering the interior. Its interior floor has also moved to the project-authored `assets/world/tblack/digilab/floor/floor-1.png` and `floor-2.png` assets. Those 1024×1024 sources keep dedicated top-face UVs and are modularized as authored 2×2 source panels: each quadrant is sampled independently onto one exact 64×32 runtime diamond. This preserves the original grid size without crushing the complete source panel into every tile; only the remaining service interiors still use Devil's Work.shop surfaces. Hospital, Training, Data Market and Archive still use labeled floor pads until their dedicated structure artwork is authored.
