...
The complete 1024x1024 source library remains under `assets_1024x1024/` behind `.gdignore`. Only selected runtime surfaces are copied into `city_1024/`, so Web/APK exports do not import the entire source collection. Source 0054 remains available as a neutral paving reference, but Central City's exterior circulation is now rendered by the procedural micro-paver field. Raised service foundations and planted islands define the street network through negative space instead of painting a second tile-scale path layer.

MCBlocks has been retired from DigiGame. DigiLab is the first Central City service to receive a dedicated exterior (`assets/world/tblack/digilab/digilab.png`); its complete exterior uses a measured scale/rotation correction so the authored structure follows the exact 64x32 city axes, while its door remains aligned to a 0054 forecourt and drives the seamless interior threshold. The dedicated `digilab/` folder also carries `digilab-door-semi-open.png` and `digilab-door-open.png`, which are played in sequence before entering the interior. Its interior floor now renders only the project-authored `assets/world/tblack/digilab/floor/floor-1.png` surface. The complete 1024×1024 top face keeps its dedicated UVs and is sampled onto each exact 64×32 runtime diamond, so the DigiLab stays visually uniform and the gameplay grid remains unchanged. `floor-2.png` stays in the project as authored source art but is not currently rendered; only the remaining service interiors still use Devil's Work.shop surfaces. Training uses the dedicated `assets/world/tblack/training-center/training-center.png` exterior with a measured doorway/footprint and corrected 64x32 isometric projection. Digi Hospital uses the project-supplied `assets/world/tblack/hospital/hospital.png` exterior with a uniform render scale that preserves the authored building and doorway proportions; only its heading receives a small rotation correction to align with the city isometric grid. DigiLab, Training and Hospital now sit on light raised civic foundations derived from their measured footprints while the surrounding medium-gray micro-paver field remains the circulation layer. Data Market and Archive still use labeled floor pads until their dedicated structure artwork is authored.
