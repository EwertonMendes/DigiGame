...
MCBlocks has been retired from DigiGame. DigiLab is the first Central City service to receive a dedicated exterior (`assets/world/tblack/digilab/digilab.png`); its authored door is aligned to a 0054 forecourt and drives the seamless interior threshold. The dedicated `digilab/` folder also carries `digilab-door-semi-open.png` and `digilab-door-open.png`, which are played in sequence before entering the interior. Its interior floor now renders only the project-authored `assets/world/tblack/digilab/floor/floor-1.png` surface. The complete 1024×1024 top face keeps its dedicated UVs and is sampled onto each exact 64×32 runtime diamond, so the DigiLab stays visually uniform and the gameplay grid remains unchanged. `floor-2.png` stays in the project as authored source art but is not currently rendered; only the remaining service interiors still use Devil's Work.shop surfaces. Hospital, Training, Data Market and Archive still use labeled floor pads until their dedicated structure artwork is authored.
