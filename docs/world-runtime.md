...
`CentralCityDecor.gd` then loads the replaceable catalog/profiles in `assets/resources/world/central_city_decor.json`. Runtime props live under `assets/world/tblack/city/props_v2/` as nearest-filtered transparent PNG pixel art; the rejected SVG prototype kit is not used. Placements use fractional logical coordinates and are composed around destinations and lot edges. Small ground footprints register directly with existing polygon walkability. Stable asset IDs allow later art replacement by changing texture, anchor, scale or blocker metadata without rewriting district placement code.
