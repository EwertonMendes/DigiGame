...
Central City's **hardscape top surface is procedural** rather than sourced from one large visual tile per gameplay cell. Neutral lots and civic paths share the same fine paver shader, but paths now have their own brighter warm-gray surface batch. A two-cell-wide cross-city boulevard, section-edge sidewalks and smaller district paths create a connected pedestrian network from the central plaza to services, parks, market, archive, canal bridges and gates. District identity is carried by architecture, lighting and signage rather than giant brown/purple floor pads; deterministic per-stone and broad-area tonal variation still prevents the result from reading as a checkerboard. Grass, planted terraces and digital water continue to use the curated Devil's Work.shop source art, and perimeter cells still render the original block side faces so the island edge keeps real authored depth. The city remains an octagonal digital island with clipped corners, with the same main cross-city promenade, block sidewalks, district lots, civic plaza, park blocks and bridged canal.
...
The current foundation kit lives under `assets/world/tblack/city/props/` and covers deeper isometric seating, flower/shrub planters, long landscape beds, shorter cyan/warm streetlights, public terminals, framed holographic wayfinding, bins, bollards and canal railings. Props are composed around path edges and destinations instead of being distributed uniformly through empty pavement. Static props have no per-frame scripts; holographic signs use one lightweight canvas shader. Small screen-space ground footprints are registered directly with the area's existing polygon walkability instead of blocking whole cells or adding a physics body per prop. Asset IDs remain stable in the JSON catalog so final pixel-art replacements can change texture, anchor, scale and footprint without rewriting district placement code.
