...
Central City's **hardscape top surface is procedural** rather than sourced from one large visual tile per gameplay cell. Neutral streets, promenades, market paving, gate paving and the muted tech district variants share the same fine paver shader and differ through a restrained surface palette; deterministic per-stone and broad-area tonal variation prevents the result from reading as a checkerboard. Grass, planted terraces and digital water continue to use the curated Devil's Work.shop source art, and perimeter cells still render the original block side faces so the island edge keeps real authored depth. The city remains an octagonal digital island with clipped corners, with the same main cross-city promenade, block sidewalks, district lots, civic plaza, park blocks and bridged canal.
