...
func _assert_seam_crossing(
	...
	assert(
		streamer.call("world_to_chunk", player.global_position) == expected_chunk,
		"Crossing the %s seam must transfer ownership to %s" % [direction_name, str(expected_chunk)]
	)
