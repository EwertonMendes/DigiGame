...
func _ready() -> void:
	...
	assert(not entry_thresholds.is_empty(), "Loaded service districts must expose physical access thresholds")
	...
	assert(bool(manager.call("is_active")), "Crossing a service access point must enter its dedicated interior")
	...
	assert(player.global_position.is_equal_approx(expected_return), "Exit must restore the authored exterior access position")
