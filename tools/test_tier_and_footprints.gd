...
func _test_ascension_and_expansion(database: DigimonDatabase, factory: DigimonFactory) -> void:
	...
	_assert(is_equal_approx(float(target.current_hp) / float(expanded_hp_max), 0.5, 0.01), "Expansion must preserve current HP proportion")
	_assert(is_equal_approx(float(target.current_mp) / float(normal_sp_max), 0.5, 0.01), "Expansion must preserve current SP proportion")
