...
func _test_party_storage_and_admission(database: DigimonDatabase, factory: DigimonFactory, stats: DigimonStatCalculator, hospital: HospitalService) -> void:
	...
	assert(not hospital.preview(storage_digimon, max_hp, 9999, 5000, collection.get_location(storage_digimon.id)).get("can_admit", true), "Storage Digimon must not be admissible from Hospital UI")
...
func _test_offline_completion_waits_for_discharge(database: DigimonDatabase, factory: DigimonFactory, stats: DigimonStatCalculator, hospital: HospitalService) -> void:
	...
	assert(not hospital.complete_if_ready(patient, max_hp, completes_at - 1, PlayerCollection.LOCATION_HOSPITAL), "Recovery must not complete early")
	assert(hospital.complete_if_ready(patient, max_hp, completes_at, PlayerCollection.LOCATION_HOSPITAL), "Offline timestamp completion must restore HP")
	assert(patient.current_hp == max_hp and patient.has_hospital_recovery(), "Completed recovery must keep its interval until discharge")
	...
	assert(not hospital.complete_if_ready(patient, max_hp, completes_at + 1, PlayerCollection.LOCATION_HOSPITAL), "Repeated completion processing must be idempotent")
...
func _test_instant_recovery_is_atomic_and_idempotent(database: DigimonDatabase, factory: DigimonFactory, stats: DigimonStatCalculator, hospital: HospitalService) -> void:
	...
	var expected_cost := int(hospital.preview(patient, max_hp, collection.bits, 8000, PlayerCollection.LOCATION_PARTY).get("instant_cost", 0))
	var before_bits := collection.bits
	var result := hospital.recover_now(collection, patient, max_hp, 8000)
	...
	assert(collection.bits == before_bits - expected_cost and patient.current_hp == max_hp, "Immediate recovery must deduct Bits once and restore HP")
	...
	var repeated := hospital.recover_now(collection, patient, max_hp, 8000)
	...
	var poor_max_hp := stats.get_stat(poor_patient, database.get_by_seed(poor_patient.species_seed), "hp")
	...
	var failed := hospital.recover_now(poor_collection, poor_patient, poor_max_hp, 8000)
